...
CREATE TABLE `t_biz_device` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC COMMENT='设备';
...
/*Table structure for table `t_biz_parts` */

DROP TABLE IF EXISTS `t_biz_parts`;

CREATE TABLE `t_biz_parts` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
...
/*Data for the table `t_biz_parts` */

/*Table structure for table `t_biz_parts_his` */

DROP TABLE IF EXISTS `t_biz_parts_his`;

CREATE TABLE `t_biz_parts_his` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `create_user` varchar(100) NOT NULL COMMENT '创建人',
  `img_id` varchar(100) DEFAULT NULL COMMENT '图片id',
...
/*Data for the table `t_biz_parts_his` */
...
CREATE TABLE `t_biz_wiringdiagram` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC COMMENT='接线图';
...
BEGIN
	DECLARE ret VARCHAR(200);
	DECLARE done INT DEFAULT FALSE;
	DECLARE cur CURSOR  FOR SELECT login_name FROM t_auth_user WHERE id = userid;
	DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;		
	SET ret = '';
	OPEN cur;
		FETCH cur INTO ret;
